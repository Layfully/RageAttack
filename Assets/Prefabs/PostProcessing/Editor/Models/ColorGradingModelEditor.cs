...
namespace UnityEditor.PostProcessing
{
    ...
    {
        static readonly GUIContent[] s_Tonemappers =
        {
            new GUIContent("None"),
        ...
        static readonly GUIContent[] s_Curves =
        {
            new GUIContent("YRGB"),
        ...
        static Material _sMaterialSpline;
        ...
        private void DoCurvesGui()
        {
            ...
            using (new EditorGUI.DisabledGroupScope(serializedProperty.serializedObject.isEditingMultipleObjects))
            {
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    ...
                    if (_sMaterialSpline == null)
                        _sMaterialSpline = new Material(Shader.Find("Hidden/Post FX/UI/Curve Background")) { hideFlags = HideFlags.HideAndDontSave };
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    ...
                    if (selection.curve != null && selection.keyframeIndex > -1)
                    {
                        ...
                        infoRect.width = 100f;
                        infoRect.height = 30f;
                        GUI.Label(infoRect, string.Format("{0:F3}\n{1:F3}", key.time, key.value), FxStyles.preLabel);
                    }
                }
            }
...
- [Alt] + [Double click] to create a key on the curve(s) at a given time",
        ...
        void DrawBackgroundTexture(Rect rect, int pass)
        {
            ...
            _sMaterialSpline.SetFloat("_DisabledState", GUI.enabled ? 1f : 0.5f);
            _sMaterialSpline.SetFloat("_PixelScaling", EditorGUIUtility.pixelsPerPoint);

            Graphics.Blit(null, rt, _sMaterialSpline, pass);
